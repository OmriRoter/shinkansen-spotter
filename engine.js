...
(function (root, factory) {
  ...
  function blockingTrain(events, t, dir) {
    ...
    for (const e of events) {
      ...
      // Researched dwell times: a fast train's normal stop is ~0.5–2 min, while a local
      // (Kodama/Tsubame/…) waits ~3–6 min at a station to be overtaken — exactly the
      // blocking case — so locals get a 5-min occupancy window.
      const cls = meta(e.type).cls;
      const dwell = cls === "local" ? 5 : cls === "rapid" ? 2 : 1.5;
      const margin = 0.5;
